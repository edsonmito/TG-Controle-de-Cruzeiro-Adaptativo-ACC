...
## 1. Modelando a Programação Quadrática (QP)
...
- **$p_\delta \delta^2$**: Penaliza a relaxação da CLF. O peso $p_\delta$ é altíssimo (ex: $10^5$), então o QP usa $\delta > 0$ apenas em emergência.
...
**Limites de Atuação (Opcional, adicionando conforto)**
...
## 7. Interpretação Física do Sistema de Equações
...
**Nota Final:** Este sistema de equações é a base teórica implementada no modelo Simulink (`ACC_TESTE_2026`) e resolvida numericamente pelo algoritmo de Hildreth (`QPhild.m`). 
