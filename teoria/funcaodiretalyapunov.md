...
Como $-2x^4 < 0$ para todo x ≠ 0, concluímos que o equilíbrio x = 0 é **assintoticamente estável** — e chegamos a essa conclusão sem resolver a equação diferencial!
...
Esta formulação matemática permite projetar leis de controle não-lineares universais (como a famosa Fórmula de Sontag) e é a base da otimização em tempo real via Programação Quadrática (QP) utilizada no projeto de ACC.
